var shop = {
	...
	userInit: function(name, connection) {
		...
		if (wallet.items.symbol) {
			...
			user.getIdentity = function (roomid) {
					...
					return wallet.items.symbol.val + name;
			};
			user.updateIdentity();
	...
	openWallets: function() {
		...
		fs.readFile('./config/wallets.txt', function(err, data) {
			...
			//import wallets
					...
					//"johtobot":{"money":87,"items":{"glow":{"name":"glow","val":"red","lastPayment":1403095657559,"daily":15},"symbol":{"name":"Symbol","val":"M","lastPayment":123456789,"daily":3}},"hours":87,"minutes":46}
					//^that === johtobot^[87[ITEMS[87[46@
	...
	newWallet: function(uid) {
		var uid = toId(uid);
		if (!Users.get(uid)) return "That user doesn't exist.";
		this.wallets[uid] = {
			money: 0,
			items: new Object(),
			hours: 0,
	...
	giveMoney: function(name, amount, connection) {
		...
		var socket;
		if (Users.get(uid)) socket = Users.get(uid);
	...
	commands: {
		...
		seen: function(target, room, user) {
			...
			var tar = user;
			if (target) tar = Users.get(target);
			if (!tar) return this.sendReply('user does not exist.');
			var uid = tar.userid;
		...
		profile: function(target, room, user) {
			...
			insides += '&nbsp;&nbsp;<font color="blue"><b>Name:</b></font> ' + name;
			insides += '<br />&nbsp;&nbsp;<font color="blue"><b>Group:</b></font> ' + group;
			insides += '<br />&nbsp;&nbsp;<font color="blue"><b>Last Seen:</b></font> ' + lastSeen;
			insides += '<br />&nbsp;&nbsp;<font color="blue"><b>Money:</b></font> ' + money;
			insides += '<br />&nbsp;&nbsp;<font color="blue"><b>Items:</b></font> ' + items;
			insides += '<br /><br />';
